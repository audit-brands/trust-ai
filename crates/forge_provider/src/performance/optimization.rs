...
impl ModelLoadingOptimizer {
    ...
    pub fn new(config: OptimizationConfig) -> Self {
        ...
        Self { config, cache: Arc::new(RwLock::new(cache)), preloader }
    }
    ...
    async fn apply_model_preloading(
        ...
        let related_models = self
            .preloader
            .get_related_models(provider_name, model_name)
            .await;
...
impl ModelPreloader {
    ...
    async fn get_related_models(&self, provider_name: &str, model_name: &str) -> Vec<String> {
        ...
        // Simple heuristic: return models from the same provider that are frequently
        // used
        let mut related_models = Vec::new();

        for (model_key, frequency) in &patterns.model_frequency {
            if model_key.starts_with(provider_name)
                && !model_key.ends_with(model_name)
                && *frequency > 5
            {
        ...
        related_models
            .sort_by_key(|key| std::cmp::Reverse(patterns.model_frequency.get(key).unwrap_or(&0)));
...
impl ResourceMonitor {
    ...
    pub async fn is_under_pressure(&self) -> bool {
        ...
        usage.memory_usage_percent / 100.0
            > self.config.memory_optimization.memory_pressure_threshold
    ...
    pub async fn get_resource_recommendations(&self) -> Vec<ResourceRecommendation> {
        ...
        if usage.cpu_usage_percent > 90.0 {
            recommendations.push(ResourceRecommendation {
                ...
                suggested_action: "Consider reducing worker threads or enabling CPU throttling"
                    .to_string(),
            });
        }
        ...
        if usage.disk_usage_percent > 90.0 {
            recommendations.push(ResourceRecommendation {
                ...
                suggested_action: "Consider cleaning up model cache or reducing cache size"
                    .to_string(),
            });
        }
...
mod tests {
    ...
    fn test_performance_improvement_merge() {
        ...
        assert_eq!(
            improvement1.response_time_improvement,
