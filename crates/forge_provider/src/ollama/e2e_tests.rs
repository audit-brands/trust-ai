...
async fn test_ollama_end_to_end_integration() -> anyhow::Result<()> {
    ...
    let mut test_suite = OllamaIntegrationTest::new()
        .await
...
async fn test_ollama_error_handling() -> anyhow::Result<()> {
    ...
    let reqwest_error = reqwest::Error::from(reqwest::Error::from(std::io::Error::new(
...
async fn test_ollama_performance_benchmark() -> anyhow::Result<()> {
    ...
    let context =
        Context::default().add_message(ContextMessage::user("Test message", model_id.into()));
