...
use forge_provider::discovery::{ModelDiscoveryService, ModelDiscoveryResult};
use forge_provider::health::HealthMonitor;
use forge_provider::selection::{ProviderSelector, SelectionContext, UserPreferences, ProviderType};
...
async fn test_complete_local_provider_workflow() {
    ...
    let discovery_service = ModelDiscoveryService::new(local_config.clone()).await.unwrap();
    let health_monitor = HealthMonitor::new(local_config.clone()).await.unwrap();
    let mut provider_selector = ProviderSelector::new(local_config, fallback_config).await.unwrap();
...
async fn test_multi_provider_workflow_with_health_states() {
    ...
    for (model_id, streaming, tools, description) in scenarios {
        ...
        match result {
            Ok(selection) => {
                println!("{}: Selected {} ({})", description, selection.provider_name, selection.reason);
                
                // Simulate request completion
                provider_selector.record_success(&selection.provider_name, Duration::from_millis(150));
            }
            Err(e) => {
...
async fn test_workflow_with_failure_and_recovery() {
    ...
    for attempt in 1..=5 {
        ...
        match result {
            Ok(selection) => {
                println!("Attempt {}: Selected {} (fallback: {})", 
...
async fn test_workflow_performance() {
    ...
    if !times.is_empty() {
        ...
        println!("Selection performance: avg={:?}, max={:?}", avg_time, max_time);
        
        // Performance assertions
        assert!(avg_time < Duration::from_secs(1)); // Average should be fast
        assert!(*max_time < Duration::from_secs(5)); // Even worst case should be reasonable
    }
...
async fn test_workflow_with_configuration_changes() {
    ...
    let discovery_service = ModelDiscoveryService::new(local_config.clone()).await.unwrap();
    let mut provider_selector = ProviderSelector::new(local_config.clone(), fallback_config.clone()).await.unwrap();
    provider_selector.initialize().await.unwrap();
    ...
    println!("Initial state: {} models, {} providers", 
    ...
    let new_discovery = ModelDiscoveryService::new(multi_config.clone()).await.unwrap();
    let mut new_selector = ProviderSelector::new(multi_config, fallback_config).await.unwrap();
    ...
    println!("Updated state: {} models, {} providers", 
...
async fn test_workflow_error_handling() {
    ...
    let discovery_service = ModelDiscoveryService::new(empty_config.clone()).await.unwrap();
    let health_monitor = HealthMonitor::new(empty_config.clone()).await.unwrap();
    let mut provider_selector = ProviderSelector::new(empty_config, fallback_config).await.unwrap();
