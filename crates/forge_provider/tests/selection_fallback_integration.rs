...
use forge_provider::selection::{ProviderSelector, SelectionContext, UserPreferences, ProviderType, ProviderSelection};
...
async fn test_provider_selection_with_healthy_local() {
    ...
    // Note: This test may fail if actual provider selection logic requires real health checks
    // In a real implementation, we would mock the health monitor
...
async fn test_user_preferences_variants() {
    ...
    assert_eq!(local_prefs.max_response_time.unwrap(), Duration::from_secs(10));
    ...
    assert_eq!(cloud_prefs.max_response_time.unwrap(), Duration::from_secs(30));
}
